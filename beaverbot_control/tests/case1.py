...
class Case1(object):
    ...
    def _parameter_register(self):
        ...
        self._stop_duration = 5

        self._avoiding_duration = 2
    ...
    def _avoid_obstacle(self):
        ...
        if self._avoiding_timer is None:  # Start avoidance timer only once
            self._avoiding_timer = time.time()
            self._trajectory_step = 0  # Start at the beginning of the trajectory

        rospy.loginfo("Starting obstacle avoidance...")

        while self._trajectory_step < len(self._trajectory.t):
            self._cmd_vel_message.linear.x = self._trajectory.u[0, self._trajectory_step]
            self._cmd_vel_message.angular.z = self._trajectory.u[1, self._trajectory_step]
            ...
            # If the avoidance duration is over, stop the loop
            if time.time() - self._avoiding_timer > self._avoiding_duration:
                break

    def _continue_avoiding(self):
        """! Continue the avoidance trajectory from the last saved step. """
        rospy.loginfo("Continuing avoidance...")
        ...
        while self._trajectory_step < len(self._trajectory.t):
            ...
            # If the obstacle disappears, resume normal movement
            if not self._lidar_check:
                rospy.loginfo("Path is clear, resuming normal movement...")
                self._state = "moving"
                self._avoiding_timer = None  # Reset avoidance timer
                self._trajectory_step = 0  # Reset trajectory
                return
    ...
    def robot_control(self):
        ...
        while not rospy.is_shutdown():
            ...
            rospy.loginfo("State: {}".format(self._state))
            ...
            if self._state == "moving":
                ...
                if not self._lidar_check:
                    ...
                    if time.time() - self._stop_timer > self._stop_duration:
                        
                        self._state = "avoiding"

                        self._avoiding_time = time.time()

            elif self._state == "avoiding":
                if time.time() - self._avoiding_time < self._avoiding_duration:
